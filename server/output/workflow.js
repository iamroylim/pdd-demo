...
exports.WorkflowValidator = exports.WorkflowEngine = void 0;
...
class WorkflowEngine {
     ...
     */
    async execute(steps) {
        console.log(`[${this.context.mobile || 'workflow'}] 开始执行工作流，共${steps.length}个步骤`);
        this.results = [];
        for (let i = 0; i < steps.length; i++) {
            const step = steps[i];
            const startTime = Date.now();
            try {
                console.log(`[${this.context.mobile || 'workflow'}] 执行步骤 ${i + 1}/${steps.length}: ${step.description || step.action}`);
                ...
                console.log(`[${this.context.mobile || 'workflow'}] 步骤 ${i + 1} 执行成功 (${duration}ms)`);
            }
            catch (error) {
                const duration = Date.now() - startTime;
                const result = {
                ...
                console.error(`[${this.context.mobile || 'workflow'}] 步骤 ${i + 1} 执行失败:`, error);
                // 继续执行后续步骤，不中断工作流
                console.log(`[${this.context.mobile || 'workflow'}] 跳过步骤 ${i + 1}，继续执行后续步骤`);
            }
        }
    ...
    async executeStep(step) {
        ...
        try {
            switch (step.action) {
                ...
                case 'click':
                    if (!step.item)
                        throw new Error('click操作需要指定item选择器');
                    try {
                        // 等待元素可见且可点击
                        ...
                        try {
                            ...
                            throw new Error(`点击元素失败: ${step.item} - ${error} - JS点击尝试: ${jsError}`);
                        }
                    }
                    break;
                case 'sleep':
                ...
                case 'input':
                    if (!step.item)
                        throw new Error('input操作需要指定item选择器');
                ...
                case 'wait':
                    if (!step.item)
                        throw new Error('wait操作需要指定item选择器');
                    try {
                        await page.waitForSelector(step.item, { timeout });
                    }
                    catch (error) {
                ...
                case 'focus':
                    if (!step.item)
                        throw new Error('focus操作需要指定item选择器');
                    try {
                        await page.waitForSelector(step.item, { timeout });
                        await page.focus(step.item);
                    }
                ...
                case 'hover':
                    if (!step.item)
                        throw new Error('hover操作需要指定item选择器');
                    try {
                        await page.waitForSelector(step.item, { timeout });
                ...
                case 'select':
                    if (!step.item)
                        throw new Error('select操作需要指定item选择器');
                    if (step.value === undefined)
                ...
                case 'clear':
                    if (!step.item)
                        throw new Error('clear操作需要指定item选择器');
                    try {
                        await page.waitForSelector(step.item, { timeout });
                        await page.evaluate((selector) => {
                            const element = document.querySelector(selector);
 ...
 * 便捷函数：直接执行工作流
 */
async function executeWorkflow(page, steps, mobile) {
    ...
    static validate(steps) {
        ...
        steps.forEach((step, index) => {
            switch (step.action) {
                ...
                case 'focus':
                    if (!step.item) {
                        errors.push(`步骤 ${index + 1}: ${step.action}操作需要指定item选择器`);
                    }
                    break;
                case 'input':
                    if (step.value === undefined) {
                ...
                case 'evaluate':
                    if (typeof step.value !== 'string') {
                        errors.push(`步骤 ${index + 1}: evaluate操作需要指定value为JavaScript代码字符串`);
                    }
                    break;
            }
        });

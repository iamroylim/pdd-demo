...
const getLoginWorkflow = (mobile) => [
    ...
    {
        action: 'screenshot',
        value: (mobile) => `screenshots/${mobile}_captcha.png`,
...
async function initLogin(mobile) {
    ...
    const page = await (0, utils_1.createAndConfigurePage)(browser, false);
    ...
    if (screenshotStep && typeof screenshotStep.value === 'function') {
        screenshotStep.value = screenshotStep.value(mobile);
    }
...
// 完成登录工作流步骤
const getCompleteLoginWorkflow = (mobile, code) => [
    {
        action: 'wait',
        item: '#input-code',
        timeout: 3000,
...
async function completeLogin(mobile, code) {
    ...
    try {
        ...
        // 为每个账号单独保存cookies
        (0, utils_1.saveAccountCookies)(mobile, cookies);
